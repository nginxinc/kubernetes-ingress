...
func TestGenerateTransportServerConfigForBackupServiceNGINXPlus(t *testing.T) {
	...
	got, warnings := generateTransportServerConfig(&transportServerEx, listenerPort, true, false)
	if len(warnings) != 0 {
		t.Errorf("want no warnings, got %v", warnings)
	}
...
func TestGenerateTransportServerConfigForBackupServiceNGINXOSS(t *testing.T) {
	t.Parallel()
	...
	got, warnings := generateTransportServerConfig(&transportServerEx, listenerPort, false, true)
	if len(warnings) != 0 {
		t.Errorf("want no warnings, got %v", warnings)
	}
	if !cmp.Equal(want, got) {
