	...
	tests := []struct {
	...
	}{
		{
			...
			want: policiesCfg{
				WAF: &version2.WAF{
					Enable:   "on",
					ApBundle: "/etc/app_protect/bundles/testWAFPolicyBundle.tgz",
				},
			},
		},
		{
			...
			want: policiesCfg{
				WAF: &version2.WAF{
					...
					ApBundle:            "/etc/app_protect/bundles/testWAFPolicyBundle.tgz",
					ApSecurityLogEnable: true,
					ApLogConf:           []string{"/etc/app_protect/bundles/secops_dashboard.tgz syslog:server=localhost:514"},
				},
			},
		},
	}
...
func (*fakeBundleValidator) validate(bundle string) (string, error) {
	bundle = fmt.Sprintf("/etc/app_protect/bundles/%s", bundle)
