...
type Resources struct {
	Ingresses               *apis.Ingresses
	Endpoints               *apis.Endpoints
	ConfigMaps              *apis.ConfigMaps
	Pods                    *apis.Pods
	TransportServers        *apis.TransportServers
	Services                *apis.Services
	VirtualServers          *apis.VirtualServers
	VirtualServerRoutes     *apis.VirtualServerRoutes
	AppProtectPolicies      *apis.AppProtectPolicies
	AppProtectLogConfs      *apis.AppProtectLogConfs
	AppProtectUserSigLister *apis.AppProtectUserSigs
	GlobalConfigurations    *apis.GlobalConfigurations
	Policies                *apis.Policies
	IngressLinks            *apis.IngressLinks
	Secrets                 *apis.Secrets
}

func (r *Resources) GetHealthChecksForIngressBackend(backend *networking.IngressBackend, namespace string) *api_v1.Probe {
	svc, err := r.GetServiceForIngressBackend(backend, namespace)
	if err != nil {
	...
	ps, err := r.Pods.ListByNamespace(svc.Namespace, labels.Set(svc.Spec.Selector).AsSelector())
	if err != nil {
...
func (r *Resources) GetPodOwnerTypeAndNameFromAddress(ns, name string) (parentType, parentName string) {
	obj, exists, err := r.Pods.GetByKey(fmt.Sprintf("%s/%s", ns, name))
	if err != nil {
...
func (r *Resources) GetServiceForUpstream(namespace string, upstreamService string, upstreamPort uint16) (*api_v1.Service, error) {
	backend := &networking.IngressBackend{
		ServiceName: upstreamService,
		ServicePort: intstr.FromInt(int(upstreamPort)),
	}
	return r.GetServiceForIngressBackend(backend, namespace)
}

func (r *Resources) getTargetPort(svcPort api_v1.ServicePort, svc *api_v1.Service) (int32, error) {
...
func (r *Resources) GetServiceForIngressBackend(backend *networking.IngressBackend, namespace string) (*api_v1.Service, error) {
	svcKey := namespace + "/" + backend.ServiceName
	svcObj, svcExists, err := r.Services.GetByKey(svcKey)
	if err != nil {
		return nil, err
	}
...
func (r *Resources) GetEndpointsForServiceWithSubselector(targetPort int32, subselector map[string]string, svc *api_v1.Service) (endps []PodEndpoint, err error) {
	ps, err := r.Pods.ListByNamespace(svc.Namespace, labels.Merge(svc.Spec.Selector, subselector).AsSelector())
	if err != nil {
	...
	svcEps, err := r.Endpoints.GetServiceEndpoints(svc)
	if err != nil {
...
func (r *Resources) GetEndpointsForSubselector(namespace string, upstream conf_v1.Upstream) (endps []PodEndpoint, err error) {
	svc, err := r.GetServiceForUpstream(namespace, upstream.Service, upstream.Port)
	if err != nil {
	...
	for _, port := range svc.Spec.Ports {
		if port.Port == int32(upstream.Port) {
			targetPort, err = r.getTargetPort(port, svc)
			if err != nil {
	...
	endps, err = r.GetEndpointsForServiceWithSubselector(targetPort, upstream.Subselector, svc)
	if err != nil {
...
func (r *Resources) GetEndpointsForUpstream(namespace string, upstreamService string, upstreamPort uint16, isNginxPlus bool) (endps []PodEndpoint, isExternal bool, err error) {
	svc, err := r.GetServiceForUpstream(namespace, upstreamService, upstreamPort)
	if err != nil {
	...
	endps, isExternal, err = r.GetEndpointsForIngressBackend(backend, svc, isNginxPlus)
	if err != nil {
...
func (r *Resources) GetEndpointsForIngressBackend(backend *networking.IngressBackend, svc *api_v1.Service, isNginxPlus bool) (result []PodEndpoint, isExternal bool, err error) {
	endps, err := r.Endpoints.GetServiceEndpoints(svc)
	if err != nil {
	...
	result, err = r.getEndpointsForPort(endps, backend.ServicePort, svc)
	if err != nil {
...
func (r *Resources) getEndpointsForPort(endps api_v1.Endpoints, ingSvcPort intstr.IntOrString, svc *api_v1.Service) ([]PodEndpoint, error) {
	var targetPort int32
	var err error
			...
			if port.Port == targetPort {
				...
				for _, address := range subset.Addresses {
					...
					if address.TargetRef != nil {
						parentType, parentName := r.GetPodOwnerTypeAndNameFromAddress(address.TargetRef.Namespace, address.TargetRef.Name)
