...
func (lbc *LoadBalancerController) updateTransportServersStatusFromEvents() error {
	...
	for _, tl := range lbc.transportServerLister {
		for _, obj := range tl.List() {
			...
			if err != nil {
				allErrs = append(allErrs, fmt.Errorf("trying to get events for TransportServer %v/%v: %w", ts.Namespace, ts.Name, err))
				break
			}
...
func (lbc *LoadBalancerController) getAppProtectLogConfAndDst(ing *networking.Ingress) ([]configs.AppProtectLog, error) {
	...
	if _, exists := ing.Annotations[configs.AppProtectLogConfDstAnnotation]; !exists {
		return apLogs, fmt.Errorf("%v requires %v in %v", configs.AppProtectLogConfAnnotation, configs.AppProtectLogConfDstAnnotation, ing.Name)
	}
	...
	if len(logDsts) != len(logConfNsNs) {
		return apLogs, fmt.Errorf("validating App Protect Destination and Config for Ingress %v: LogConf and LogDestination must have equal number of items", ing.Name)
	}
	...
	for i, logConfNsN := range logConfNsNs {
		...
		if err != nil {
			return apLogs, fmt.Errorf("retrieving App Protect Log Config for Ingress %v: %w", ing.Name, err)
		}
...
func (lbc *LoadBalancerController) getAppProtectPolicy(ing *networking.Ingress) (apPolicy *unstructured.Unstructured, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("retrieving App Protect Policy for Ingress %v: %w", ing.Name, err)
	}
...
func (lbc *LoadBalancerController) getEndpointsForUpstream(namespace string, upstreamService string, upstreamPort uint16) (endps []podEndpoint, isExternal bool, err error) {
	...
	if err != nil {
		return nil, false, fmt.Errorf("getting service %v: %w", upstreamService, err)
	}
	...
	if err != nil {
		return nil, false, fmt.Errorf("retrieving endpoints for the service %v: %w", upstreamService, err)
	}
...
func (lbc *LoadBalancerController) getEndpointsForSubselector(namespace string, upstream conf_v1.Upstream) (endps []podEndpoint, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("getting service %v: %w", upstream.Service, err)
	}
	...
	for _, port := range svc.Spec.Ports {
		if port.Port == int32(upstream.Port) {
			...
			if err != nil {
				return nil, fmt.Errorf("determining target port for port %v in service %v: %w", upstream.Port, svc.Name, err)
			}
			break
		}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("retrieving endpoints for the service %v: %w", upstream.Service, err)
	}
...
func (lbc *LoadBalancerController) getEndpointsForServiceWithSubselector(targetPort int32, subselector map[string]string, svc *api_v1.Service) (endps []podEndpoint, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("getting pods in namespace %v that match the selector %v: %w", svc.Namespace, labels.Merge(svc.Spec.Selector, subselector), err)
	}
...
func (lbc *LoadBalancerController) getEndpointsForPort(endps api_v1.Endpoints, backendPort networking.ServiceBackendPort, svc *api_v1.Service) ([]podEndpoint, error) {
	...
	for _, port := range svc.Spec.Ports {
		if (backendPort.Name == "" && port.Port == backendPort.Number) || port.Name == backendPort.Name {
			targetPort, err = lbc.getTargetPort(port, svc)
			if err != nil {
				return nil, fmt.Errorf("determining target port for port %v in Ingress: %w", backendPort, err)
			}
			break
		}
	}
...
func (lbc *LoadBalancerController) getTargetPort(svcPort api_v1.ServicePort, svc *api_v1.Service) (int32, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("getting pod information: %w", err)
	}
	...
	if err != nil {
		return 0, fmt.Errorf("finding named port %v in pod %s: %w", svcPort, pod.Name, err)
	}
