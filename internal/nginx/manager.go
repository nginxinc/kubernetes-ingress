...
func (lm *LocalManager) Start(done chan error) {
	if lm.nginxPlus {
		...
		if versionErr != nil {
			nl.Errorf(ln.logger, "Error determining whether nginx version is >= r33: %v", versionErr)
		}
		if isR33OrGreater {
...
func (lm *LocalManager) Quit() {
	...
	if lm.nginxPlus {
		...
		if err != nil {
			nl.Errorf(nl.logger, "Error determining whether nginx version is >= r33: %v", err)
		}
