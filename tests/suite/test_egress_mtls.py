...
class TestEgressMtlsPolicyVSR:
    @pytest.mark.parametrize(
        "policy_src, vs_src, vsr_src, mtls_ca_secret, expected_code, expected_text, vsr_message, vsr_state, test_description",
        [
            (
                mtls_pol_valid_src,
                mtls_vs_vsr_src,
                ...
                "was added or updated",
                "Valid",
                "Test valid EgressMTLS policy applied to a VirtualServerRoute",
            ),
            (
                mtls_pol_valid_src,
                mtls_vs_vsr_src,
                ...
                "was added or updated",
                "Valid",
                "Test valid EgressMTLS policy applied to VirtualServerRoute with a CRL",
            ),
            (
                mtls_pol_invalid_src,
                mtls_vs_vsr_src,
    ...
    def test_egress_mtls_policy(
        ...
        """
        Test egress-mtls with valid and invalid policy in vsr subroutes.
        """
        print("------------------------- {} -----------------------------------".format(test_description))
        ...
        print(f"Patch vsr with policy: {vsr_src} and vs with tls secret: {tls_secret}")
        patch_virtual_server_from_yaml(
            kube_apis.custom_objects,
        ...
        teardown_policy(kube_apis, v_s_route_setup.namespace, tls_secret, pol_name, mtls_secret)
        ...
        assert (
            resp.status_code == expected_code
            and expected_text in resp.text
            and vsr_message in vsr_events["status"]["message"]
            and vsr_events["status"]["state"] == vsr_state
        )