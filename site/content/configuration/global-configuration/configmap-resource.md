...
|ConfigMap Key | Description | Default | Example |
| ---| ---| ---| --- |
|*zone-sync* | Enables zone synchronization between NGINX Ingress Controller Pods. This autogenerates a `zone_sync_server` and headless service using the `IngressClass` name. Please note that this headless service will be cleaned up when the deployment is uninstalled as well as if there are multiple deployments using the same `IngressClass`, these deployments will use the same headless service.  NGINX Plus Required. | *False* |  |
