...
`controller.services.create` | Creates a service to expose the Ingress Controller pods. | true
`controller.services.service[].type` | The type of service to create for the Ingress Controller. | LoadBalancer
`controller.services.service[].externalTrafficPolicy` | The externalTrafficPolicy of the service. The value Local preserves the client source IP. | Local
`controller.services.service[].annotations` | The annotations of the Ingress Controller service. | {}
`controller.services.service[].extraLabels` | The extra labels of the service. | {}
`controller.services.service[].loadBalancerIP` | The static IP address for the load balancer. Requires `controller.service.type` set to `LoadBalancer`. The cloud provider must support this feature. | ""
`controller.services.service[].externalIPs` | The list of external IPs for the Ingress Controller service. | []
`controller.services.service[].loadBalancerSourceRanges` | The IP ranges (CIDR) that are allowed to access the load balancer. Requires `controller.service.type` set to `LoadBalancer`. The cloud provider must support this feature. | []
`controller.services.service[].name` | The name of the service. | Autogenerated
`controller.services.service[].customPorts` | A list of custom ports to expose through the Ingress Controller service. Follows the conventional Kubernetes yaml syntax for service ports. | []
`controller.services.service[].httpPort.enable` | Enables the HTTP port for the Ingress Controller service. | true
`controller.services.service[].httpPort.port` | The HTTP port of the Ingress Controller service. | 80
`controller.services.service[].httpPort.nodePort` | The custom NodePort for the HTTP port. Requires `controller.service.type` set to `NodePort`. | ""
`controller.services.service[].httpPort.targetPort` | The target port of the HTTP port of the Ingress Controller service. | 80
`controller.services.service[].httpsPort.enable` | Enables the HTTPS port for the Ingress Controller service. | true
`controller.services.service[].httpsPort.port` | The HTTPS port of the Ingress Controller service. | 443
`controller.services.service[].httpsPort.nodePort` | The custom NodePort for the HTTPS port. Requires `controller.service.type` set to `NodePort`.  | ""
`controller.services.service[].httpsPort.targetPort` | The target port of the HTTPS port of the Ingress Controller service. | 443
